...
import jnarasapi.jna.WinRas.RASCONN;
import jnarasapi.jna.WinRas.RASCREDENTIALS;
import jnarasapi.jna.WinRas.RASDIALPARAMS;
import jnarasapi.jna.WinRas.RASENTRY;
import jnarasapi.jna.WinRas.RASPPPIP;
import jnarasapi.jna.WinRas.RasDialFunc2;
...
public abstract class Rasapi32Util {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(Rasapi32Util.class);
    ...
    public static HANDLE getRasConnection(String connName) throws Ras32Exception {
		LOGGER.info("getRasConnection, connName: '{}'", connName);
        ...
        for (int i = 0; i < lpcConnections.getValue(); i++) {
			...
			connections[i].dwSize = lpcb.getValue() / lpcConnections.getValue();
		}
//        lpcb  = new IntByReference(connections[0].dwSize * lpcConnections.getValue());

		connections = (WinRas.RASCONN[]) connections[0].toArray(connections);
        ...
        for (int i = 0; i < lpcConnections.getValue(); i++) {
			...
			LOGGER.info("Current connection dwSize: {}", connections[i].dwSize);
			LOGGER.info("Current connection hrasconn: {}", connections[i].hrasconn);

			LOGGER.info("Current szDeviceType: '{}'", new String(connections[i].szDeviceType));
			LOGGER.info("Current szDeviceName: '{}'", new String(connections[i].szDeviceName));
			LOGGER.info("Current szPhonebook: '{}'", new String(connections[i].szPhonebook));

			LOGGER.info("Compare connection, current: '{}'", new String(connections[i].szEntryName));

            if (new String(connections[i].szEntryName).trim().equals(connName)) {
				return connections[i].hrasconn;
			}
        }

		LOGGER.info("No matching connection available.");
